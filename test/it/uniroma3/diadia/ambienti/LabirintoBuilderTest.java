...
public class LabirintoBuilderTest {
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
	    Labirinto monolocale = Labirinto.newBuilder()            .addAttrezzo("spada", 1)
	            .addAttrezzo("spada", 1) // Questo non deve essere aggiunto perché duplicato
	            .getLabirinto();
	            
	    
	    assertEquals(1, monolocale.getStanzaIniziale().getAttrezzi().size());
	    
	    
	    Attrezzo attrezzoAtteso = new Attrezzo("spada", 1);
	    Attrezzo attrezzoEffettivo = monolocale.getStanzaIniziale().getAttrezzo("spada");
	    
	    assertEquals(attrezzoAtteso, attrezzoEffettivo);
	}
	...
	// sostituisci le direzioni e labirinto builder come sopra>>>>>>
	...
	public void testAggiuntaAttrezziAStanze_AppenaAggiunte() {
		...
		Labirinto.newBuilder()
				...
				.addAttrezzo(nomeAttrezzo, peso)
				.getLabirinto();
		assertTrue(this.Labirinto.newBuilder().getListaStanze().get(nomeStanza).getAttrezzi().containsValue(new Attrezzo (nomeAttrezzo,peso)));
		assertEquals(new Attrezzo(nomeAttrezzo,peso),this.Labirinto.newBuilder().getListaStanze().get(nomeStanza).getAttrezzo(nomeAttrezzo));
	}
	...
	public void testAggiuntaAttrezzoAStanze_AppenaAggiunteMultiple() {
		...
		Labirinto.newBuilder()
				...
				.getLabirinto();
		Attrezzo attrezzo = new Attrezzo(nomeAttrezzo,peso);
		Map<String, Attrezzo> attrezzi = Labirinto.newBuilder().getListaStanze().get(nomeStanza).getAttrezzi();
	...
	public void testAggiuntaAttrezzoAStanze_MoltepliciAttrezziStessaStanza() {
		...
		Map<String, Stanza> listaStanze = Labirinto.newBuilder().getListaStanze();
	...
	public void testLabirintoConStanzaMagica() {
		...
		Labirinto.newBuilder()
		.addStanzaMagica(nomeStanzaMagica);
		StanzaMagica stanzaMagica = (StanzaMagica)Labirinto.newBuilder().getListaStanze().get(nomeStanzaMagica);
	...
	public void testLabirintoConStanzaMagica_AggiuntaElementoOltreSogliaMagica() {
		String nomeAttrezzo1 = "attrezzo 1";
		String nomeAttrezzo2 = "attrezzo 2";
		String nomeAttrezzo3 = "attrezzo 3";
		String nomeAttrezzo4 = "attrezzo 4";
		String nomeAttrezzo4Inv = "4 ozzertta";
		//int sogliaMagica = 3;
		int peso1 = 1;
		int peso2 = 2;
		int peso3 = 3;
		int peso4 = 4;
		int peso4_x2 = peso4*2;
		String nomeStanzaMagica = "Stanza Magica";
		Labirinto.newBuilder()
		.addStanzaMagica(nomeStanzaMagica)
		.addAttrezzo(nomeAttrezzo1, peso1)
		.addAttrezzo(nomeAttrezzo2, peso2)
		.addAttrezzo(nomeAttrezzo3, peso2)
		.addAttrezzo(nomeAttrezzo4, peso2);
		Map<String, Stanza> listaStanze = Labirinto.newBuilder().getListaStanze();
		assertEquals(new Attrezzo(nomeAttrezzo4Inv,peso4_x2), listaStanze.get(nomeStanzaMagica).getAttrezzo(nomeAttrezzo4Inv));
	...
	public void testLabirintoConStanzaBloccata_ConPassepartout() {
		...
		assertEquals(stanzaVincente,Labirinto.newBuilder().getListaStanze().get("stanza bloccata").getStanzaAdiacente("nord"));	
	}
	...
	public void testLabirintoConStanzaBloccata_SenzaPassepartout() {
		...
		assertNull(Labirinto.newBuilder().getListaStanze().get("stanza bloccata").getStanzaAdiacente("nord"));
	}
	
	@Test
	public void testLabirintoCompletoConTutteLeStanze() {
		
		Labirinto labirintoCompleto = this.Labirinto.newBuilder()
				.addStanzaIniziale(nomeStanzaIniziale)
				.addStanzaVincente(nomeStanzaVincente)
				.addStanza("corridoio")
		...
		assertTrue(corridoio.getMapStanzeAdiacenti().keySet().containsAll(Arrays.asList("ovest","est","nord","sud")));
		...
		assertEquals(2, corridoio.getNumeroAttrezzi());
	    assertTrue(corridoio.hasAttrezzo("chiave"));
	    assertTrue(corridoio.hasAttrezzo("lanterna"));
	    assertEquals(1, corridoio.getAttrezzo("chiave").getPeso());
	    assertEquals(1, corridoio.getAttrezzo("lanterna").getPeso());
	}
